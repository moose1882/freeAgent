...
//start outer channel loop. General data about the RSS channel
	//$namespaces = $RSS_DOC->getNamespaces(true); // get namespaces
...
//start inner channel->image loop. data for channel image
		foreach ($RSS_DOC->channel->image as $RSSimage)
			...
			//	$image_link_media = $RSSchannel->children('media', True)->content->attributes();
			//	$image_link_media = $xml->channel->RSSimage[$i]->children('media', True)->content->attributes();
...
//start inner channel-->item loop		
		foreach($RSS_DOC->channel->item as $RSSitem)
			{
	//Grab media:content 
		//	$media_url = trim((string)$RSSitem->children($namespaces['media'])->content->attributes()->url);
		//	echo $media_url;
	    //$ns_media = $RSSitem->children('http://www.rssboard.org/media-rss');

		/*     echo $ns_media->content; // displays "<media:content>"
				if (isset($RSSitem->{'media:content'})) {
					echo "We have media!: ";
					$url = (string)$RSSitem->{'media:content'}->attributes()->url;
					$description = (string)$RSSitem->{'media:content'}->{'media:title'};
					echo "$url:" & $url;
					log_it ($log_date. "media url: " .$url);
					echo "$description: " & $description;
					log_it ($log_date. "media description: " .$description);
				} */
...
// TO DO: add date check to only add rss articles from last 6 months?
...
{
	$logfile = fopen("/var/www/html/beta/logs/ingest_logs.txt", "a") or die("Unable to open ingest log file!\n");
